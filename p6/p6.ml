...
let combine l1 l2 = List.map2 (function a -> function b -> a,b) l1 l2;;
...
let partition f l = (List.filter f l),(List.filter (fun l -> if f l then false else true) l);; 
...
let multicomp lf x = List.fold_left (fun acc f -> f acc) x lf;;
